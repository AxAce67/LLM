...
def resolve_mix_run_paths(work_dir: Path) -> dict[str, Path]:
    return {
        "manifest": work_dir / "manifests" / "pretrain_mix_ja.jsonl",
        "manifest_report": work_dir / "manifests" / "pretrain_mix_ja.report.json",
...
def run_pretrain_mix_pipeline(
    *,
    ...
    if not manifest_inputs and not skip_merge:
    ...
    if skip_merge:
        _require_exists(paths["manifest"], "merged manifest")
        _require_exists(paths["manifest_report"], "merged manifest report")
    else:
        merge_report = merge_manifests(inputs=manifest_inputs, output=paths["manifest"], min_chars=min_chars)
        paths["manifest_report"].write_text(json.dumps(merge_report, ensure_ascii=False, indent=2), encoding="utf-8")
        steps.append("merge")

    manifest_report = json.loads(paths["manifest_report"].read_text(encoding="utf-8"))
    if int(manifest_report.get("kept_docs", 0)) <= 0:
        raise ValueError("Merged manifest produced zero documents")
