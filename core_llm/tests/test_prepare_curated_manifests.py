...
def test_prepare_curated_manifests_builds_expected_outputs(tmp_path: Path):
    ...
    gov_dir = raw_root / "government_ja"
    ...
    (gov_dir / "policy1.txt").write_text(
        "これは公的オープンデータ由来の日本語説明文です。こちらも十分に長い本文として扱います。",
        encoding="utf-8",
    )
    ...
    gov_manifest = manifest_dir / "government_ja.jsonl"
    assert notes_manifest.exists()
    assert tech_manifest.exists()
    assert gov_manifest.exists()
